...
mass_dict = {
    znum: mass
    for znum, mass in enumerate([
        0., 1.00811, 4.002602, 6.997, 9.0121831, 10.821, 12.0116, 14.00728,
        15.99977, 18.998403, 20.1797, 22.990, 24.305, 26.982, 28.085, 30.974,
        32.06, 35.45, 39.948
    ])
}
chem_dict = {
...
class VerletPropagator():
    ...
    def _propagate_coords_after_first_ts(self, coords, coords_nm1, masses,
                                         forces):
    ...
    def propagate(self, particles, time_step):
        ...
        masses = particles.masses
        if time_step == 0:
            return self._propagate_coords_first_ts(coords, velocs, masses,
                                                   forces)
        else:
...
class LJPotential():
    ...
    def ff(self, d_vec, r_vec):
        ...
        A = self.A
        B = self.B
        scalar_force = np.divide(12 * A, d_vec**14) - np.divide(
            6 * B, d_vec**8)
...
class ParticleGroup():
    def __init__(self, coords, velocs, masses, znumbers):
        try:
            assert isinstance(znumbers, np.ndarray) and len(
                znumbers.shape) == 1
    ...
    def from_xyz(cls, input_path, velocities='xyz'):
        ...
        if velocities != 'xyz' and velocities[0] != 'maxwell':
            ...
            velocs = generate_velocities(coordinates,
                                         distribution,
                                         spread=spread)
    ...
    with open(input_path) as myfile:
        for num, line in enumerate(myfile):
            if num > 1:
                ...
                velocs.append([float(x) for x in values[5:8]])
    ...
    with open(input_path) as myfile:
        for num, line in enumerate(myfile):
            ...
            if num > 1:
                ...
                coordinates.append([float(x) for x in values[1:4]])
...
def generate_velocities(coordinates,
                        distribution,
                        spread=None,
                        masses=None,
    ...
    if distribution == 'uniform':
        velocities = genran.uniform(-spread / 2, spread / 2, coordinates.shape)
    ...
    if distribution == 'maxwell':
        ...
        for j in range(coordinates.shape[0]):
            velocities.append(genran.normal(loc=0.0, scale=variance[j],
                                            size=3))

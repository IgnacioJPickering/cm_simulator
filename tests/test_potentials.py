...
@pytest.mark.parametrize("var,expected", [(1, 1.8897261246257702),
                                          (2, 2 * 1.8897261246257702)])
def test_ang2bohr(var, expected):
    '''Test conversion from angstroem to bohr'''
    assert cm.ang2bohr(var) == expected


@pytest.mark.parametrize("var,expected", [(1, 1 / 1.8897261246257702),
                                          (2, 2 / 1.8897261246257702)])
...
def test_distance_matrix():
    ...
    assert np.allclose(np.array([[dii, d12, d13], [d12, dii, d23],
                                 [d13, d23, dii]]),
                       dij,
                       atol=1e-5,
                       rtol=1e-8)
